...
export class ClaudeService {
  ...
  constructor(private configService: ConfigService) {
    this.anthropic = new Anthropic({
      apiKey: this.configService.get<string>('ai.claude.apiKey'),
    });
  }
  ...
  async generateResponse(
    ...
    try {
      ...
      const prompt = this.buildPrompt(context, userMessage, systemPrompt);
  ...
  async generateProductRecommendations(
    ...
    try {
      ...
      this.logger.error('Claude product recommendation error:', error);
      return [];
    }
  }
  ...
  async analyzeSentiment(message: string, language: Language): Promise<{
    ...
    try {
      ...
      this.logger.error('Claude sentiment analysis error:', error);
      return {
        sentiment: 'neutral',
        confidence: 0.5,
        emotions: [],
...
Current time: ${new Date().toLocaleString('en-MY', { timeZone: 'Asia/Kuala_Lumpur' })}`;
  ...
  private buildPrompt(
    ...
    prompt += `Please respond as ${context.salesPersona.salespersonName} in ${context.language} language.`;
